...
// server
import { closeDevice } from 'src/server/device-adapters';
...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    const result = await closeDevice(device);
    ...
    const updatedDevice = await prisma.device.update({
      ...
      data: {
        ...
        jobs: {
          create: {
            type: device.type === 'ANTIDETECT_PROFILE' ? 'antidetect.closeProfile' : 'android.closeDevice',
            status: 'completed',
            payload,
            attempts: 1,
            finishedAt: new Date(),
    ...
    await prisma.jobLog.create({
      data: {
        ...
        attempts: 1,
        errorMessage: error instanceof Error ? error.message : 'Close device thất bại',
        finishedAt: new Date(),
      },
    });

    return NextResponse.json(
      { message: error instanceof Error ? error.message : 'Close device thất bại' },
      { status: 500 }
    );
  }
}
